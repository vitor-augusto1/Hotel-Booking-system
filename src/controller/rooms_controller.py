...
from decorators.auth_decorator import admin_only, login_required
...
def return_all_available_rooms_and_price_range():
    ...
    if None not in (min_price, max_price):
        rooms: Optional[
            List[Room]
        ] = db.return_all_available_rooms_by_price_range(min_price, max_price)
    else:
...
def return_available_rooms_by_type_and_price_range(room_type: RoomType):
    ...
    if None not in (min_price, max_price):
        rooms: Optional[
            List[Room]
        ] = db.return_available_rooms_by_price_range_and_type(
            min_price, max_price, RoomType[room_type.upper()].name
        )
    else:
        rooms: Optional[
            List[Room]
        ] = db.return_all_available_rooms_by_its_type(
